...
if (!process.env.SUPABASE_URL || !process.env.SUPABASE_SECRET_KEY) throw new Error('SUPABASE_URL and SUPABASE_SECRET_KEY are required in server/.env')
if (!password) throw new Error('ADMIN_PASSWORD is required for this one-time command and must not be committed')
...
if (listError) throw new Error(`Unable to access Supabase Auth: ${listError.message}`)
...
if (existingUser) {
  ...
  if (error) throw new Error(`Unable to update admin: ${error.message}`)
  ...
  if (error) throw new Error(`Unable to create admin: ${error.message}`)
